...
-- 4. BẢNG NGƯỜI DÙNG (nguoi_dung) - ĐÃ CẬP NHẬT
...
CREATE TABLE thong_bao (
    id_thong_bao UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    id_nguoi_nhan UUID REFERENCES nguoi_dung(id_nguoi_dung) ON DELETE CASCADE,
    tieu_de TEXT NOT NULL,
    noi_dung TEXT NOT NULL,
    link TEXT, -- Đường dẫn khi click vào thông báo (ví dụ: /post/id-bai-dang)
...
Bắt đầu tham gia, người dùng truy cập hệ thống và thực hiện đăng nhập bằng email định dạng @husc.edu.vn. Sau khi xác thực thành công, hệ thống chuyển hướng người dùng vào giao diện Dashboard. Trong trường hợp quên mật khẩu, người dùng có thể sử dụng chức năng "Quên mật khẩu" để yêu cầu khôi phục qua email.
Ghi chú kỹ thuật về đồng bộ phân quyền: Vì hệ thống sử dụng nền tảng Backend là Node.js thay vì sử dụng hoàn toàn Supabase Auth, Backend Node.js sẽ cần tạo ra các Custom JWT (JSON Web Tokens) tương thích với chuẩn của Supabase. Điều này đảm bảo để hệ thống Chat nội bộ (Supabase Realtime) và kịch bản so khớp ngầm (Backend API (Node.js)) vẫn có thể nhận diện được người dùng hiện tại là ai và phân quyền chính xác khi họ chat hoặc kích hoạt hàm ẩn.
Tại Dashboard, người dùng có thể Đăng tin (Mất đồ hoặc Nhặt được đồ). Thông tin gốc bao gồm tiêu đề, mô tả, vị trí, phân loại và hình ảnh. Ngay sau đó, Backend API (Node.js) sẽ chạy ngầm kịch bản so khớp và gửi thông báo nếu có bài đăng trùng khớp.
...
o   Hệ thống sử dụng các tiến trình ngầm từ Backend API (Node.js) để chạy kịch bản so khớp bài đăng tự động.
...
UC_Sub1: So khớp bài đăng tự động (Hệ thống gọi Backend API (Node.js) để tìm trùng khớp và thông báo).
...
•	UC_2: Đăng nhập hệ thống (bằng email @husc.edu.vn).
•	UC_3: Đăng xuất.
•	UC_4: Quên mật khẩu (Khôi phục mật khẩu).
Actor 1: Người dùng (Sinh viên/Cán bộ HUSC)
•	UC_5: Đăng tin Mất đồ
...
•   Mô tả ngắn gọn: Hệ thống chạy ngầm kịch bản so khớp thông qua Backend API (Node.js) ngay sau khi có bài đăng mới được tạo, nhằm tìm ra những thông tin đồ vật trùng khớp và tự động gửi thông báo cho người dùng.
•   Tác nhân chính: Hệ thống (Backend API (Node.js)).
...
2.  Backend API (Node.js) được kích hoạt và tiến hành chạy ngầm kịch bản so khớp.
...
UC_1: Đăng ký và Xác nhận Email
•	Mô tả ngắn gọn: Cho phép người dùng mới tạo tài khoản bằng email của trường và bắt buộc phải xác nhận qua email để kích hoạt, nhằm ngăn chặn việc tạo tài khoản giả mạo.
•	Tác nhân chính: Người dùng (Mọi đối tượng).
•	Tác nhân thứ cấp: Hệ thống, Máy chủ gửi Email (SMTP).
•	Tiền điều kiện: Người dùng chưa có tài khoản trên hệ thống và sở hữu một email hợp lệ có đuôi @husc.edu.vn.
•	Hậu điều kiện: Tài khoản mới được khởi tạo và kích hoạt thành công, có thể dùng để đăng nhập.
•	Kịch bản chính:
1.	Tại màn hình Đăng nhập, người dùng chọn chức năng "Đăng ký".
2.	Hệ thống yêu cầu nhập địa chỉ email (@husc.edu.vn) và mật khẩu.
3.	Người dùng điền thông tin và bấm gửi.
4.	Hệ thống gửi một liên kết xác thực (hoặc mã OTP) vào hòm thư email vừa đăng ký. Đồng thời thông báo người dùng kiểm tra email.
5.	Người dùng truy cập vào hòm thư, nhấn vào liên kết xác thực.
6.	Hệ thống xác nhận email hợp lệ, kích hoạt tài khoản và chuyển hướng người dùng đến trang Đăng nhập. Kết thúc use case.

UC_2: Đăng nhập hệ thống
...
UC_4: Quên mật khẩu (Khôi phục mật khẩu)
•	Mô tả ngắn gọn: Cho phép người dùng thiết lập lại mật khẩu mới khi không thể nhớ mật khẩu cũ bằng cách xác minh qua email.
•	Tác nhân chính: Người dùng (Mọi đối tượng).
•	Tác nhân thứ cấp: Hệ thống, Máy chủ gửi Email (SMTP).
•	Tiền điều kiện: Người dùng chưa đăng nhập, đang ở màn hình Đăng nhập và đã có tài khoản tồn tại trong hệ thống.
•	Hậu điều kiện: Người dùng đặt lại được mật khẩu mới và có thể dùng nó để đăng nhập.
•	Kịch bản chính:
1.	Tại màn hình Đăng nhập, người dùng nhấn vào "Quên mật khẩu".
2.	Hệ thống yêu cầu nhập địa chỉ email (@husc.edu.vn).
3.	Người dùng nhập email và gửi yêu cầu.
4.	Hệ thống kiểm tra email có tồn tại không và gửi một liên kết/mã xác thực khôi phục mật khẩu.
5.	Người dùng truy cập liên kết trong email, nhập mật khẩu mới và xác nhận.
6.	Hệ thống cập nhật mật khẩu mã hóa mới vào cơ sở dữ liệu và thông báo thành công. Kết thúc use case.
...
5.  <> UC_Sub1: So khớp bài đăng tự động. Ngay sau khi lưu, hệ thống tự động gọi Backend API (Node.js) để chạy ngầm kịch bản so khớp nhằm đối chiếu bài đăng mất đồ này với các bài đăng nhặt được đồ hiện có.
...
5.  <> UC_Sub1: So khớp bài đăng tự động. Ngay lập tức, hệ thống tự động gọi Backend API (Node.js) để chạy ngầm kịch bản so khớp nhằm đối chiếu bài đăng nhặt được đồ này với các bài đăng mất đồ (LOST) hiện có trên hệ thống.
...
F. Bảng Báo Cáo (bao_cao)Người dùng: Tạo báo cáo.  Admin: Xem và xử lý báo cáo.  
-- Cấp quyền: Ai cũng có thể tạo thông báo (hệ thống tự tạo)
CREATE POLICY "Cho phép thêm thông báo" ON thong_bao
    FOR INSERT WITH CHECK (true);
-- Cấp quyền: Người dùng chỉ được xem thông báo của chính mình
CREATE POLICY "Người dùng tự xem thông báo của mình" ON thong_bao
    FOR SELECT USING (auth.uid() = id_nguoi_nhan);
-- Cấp quyền: Người dùng được tự đánh dấu đã đọc (Update) thông báo của mình
CREATE POLICY "Người dùng tự cập nhật thông báo của mình" ON thong_bao
    FOR UPDATE USING (auth.uid() = id_nguoi_nhan);
